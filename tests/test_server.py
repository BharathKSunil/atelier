...
def test_cross_origin_post_is_blocked(client):
    ...
    r = client.post("/api/projects", json={},
                    headers={"X-Atelier-Token": tok, "Origin": "http://evil.example"})
    assert r.status_code == 403
...
def test_export_rejects_destination_outside_allowed_roots(app, client, tmp_path):
    ...
    tok = _token(client)
    r = client.post("/api/p/demo/persons/0/export", json={"dest": "/etc/atelier-pwn"},
                    headers={"X-Atelier-Token": tok})
...
def test_export_accepts_destination_under_allowed_root(app, client, tmp_path):
    ...
    r = client.post("/api/p/demo/persons/0/export", json={"dest": dest},
                    headers={"X-Atelier-Token": tok})
