...
def main():
    ...
    for pid, (skin, name) in enumerate(zip(PERSON_COLORS, PERSON_NAMES, strict=False)):
        conn.execute("INSERT INTO persons(id, display_name) VALUES(?,?)", (pid, name or f"Person {pid}"))
        ...
        for k, q in enumerate(quals):
            ...
            bbox, thumb = make_photo(
                path,
                skin,
                f"{name or 'Person ' + str(pid)} #{k}",
                eyes_open=(k != 3),
                sharp=(k != 3),
                bright=0.7 + 0.3 * q,
            )
            cur = conn.execute("INSERT INTO images(path, processed, width, height) VALUES(?,1,800,600)", (path,))
            iid = cur.lastrowid
            conn.execute(
    ...
    for sid, (label, skin, n) in enumerate(bursts):
        ...
        for k in range(n):
            ...
            bbox, thumb = make_photo(
                path, skin, f"{label} — frame {k}", eyes_open=not is_blink, sharp=sharp, bright=bright
            )
        ...
        conn.execute("INSERT INTO series(id, frame_count, best_image_id) VALUES(?,?,?)", (sid, n, best_iid))
